...
def summary(values: torch.Tensor) -> dict[str, float]:
    ...
    max_quantile_samples = 4_000_000
...
def roll_pitch_from_quat(quat: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
    """Return yaw-invariant XYZ roll and pitch from a world-frame body quaternion."""
    gravity_w = torch.zeros((quat.shape[0], 3), device=quat.device, dtype=quat.dtype)
    gravity_w[:, 2] = -1.0
    gravity_b = quat_apply_inverse(quat, gravity_w)
    roll = torch.atan2(-gravity_b[:, 1], -gravity_b[:, 2])
    pitch = torch.asin(torch.clamp(gravity_b[:, 0], -1.0, 1.0))
    return roll, pitch
...
def main(env_cfg, agent_cfg) -> None:
    ...
    j4340_actuators = [
        robot.actuators.get(name)
        for name in ("legs_j4340", "shoulder_pitch_roll_j4340")
    ]
    if any(actuator is None for actuator in j4340_actuators):
        raise RuntimeError("Missing a required J4340P actuator group")
    aligned_j4340_names = [
        joint_name
        for actuator in j4340_actuators
        for joint_name in actuator._joint_names
    ]
    if len(aligned_j4340_names) != len(set(aligned_j4340_names)):
        raise RuntimeError("J4340P actuator groups contain duplicate joints")
    if set(aligned_j4340_names) != set(j4340_names):
        raise RuntimeError(
            "J4340P actuator inventory differs from the 12-joint hardware contract: "
            f"actual={sorted(aligned_j4340_names)} expected={sorted(j4340_names)}"
        )
    ...
    torso_body_id = robot.body_names.index("waist_yaw_link")
    sensor_foot_ids = [
    ...
    torso_roll_all = []
    torso_pitch_all = []
    foot_lateral_spacing_all = []
    double_support_foot_lateral_spacing = []
    contact_all = []
    contact_slip = [[], []]
    contact_tilt = [[], []]
    swing_height = [[], []]
    ...
    with torch.inference_mode():
        for step in range(args_cli.steps):
            ...
            pelvis_roll, pelvis_pitch = roll_pitch_from_quat(robot.data.root_quat_w)
            torso_roll, torso_pitch = roll_pitch_from_quat(
                robot.data.body_quat_w[:, torso_body_id]
            )
            pelvis_roll_all.append(pelvis_roll.detach().clone())
            pelvis_pitch_all.append(pelvis_pitch.detach().clone())
            torso_roll_all.append(torso_roll.detach().clone())
            torso_pitch_all.append(torso_pitch.detach().clone())
            ...
            if all(hasattr(actuator, "_joint_vel") for actuator in j4340_actuators):
                effort = torch.cat(
                    [actuator.applied_effort for actuator in j4340_actuators], dim=1
                )
                velocity = torch.cat(
                    [actuator._joint_vel for actuator in j4340_actuators], dim=1
                )
            ...
            foot_delta_w = (
                robot.data.body_pos_w[:, foot_body_ids[1]]
                - robot.data.body_pos_w[:, foot_body_ids[0]]
            )
            foot_delta_b = quat_apply_inverse(robot.data.root_quat_w, foot_delta_w)
            foot_lateral_spacing = torch.abs(foot_delta_b[:, 1])
            foot_lateral_spacing_all.append(foot_lateral_spacing.detach().clone())
            double_support = contact[:, 0] & contact[:, 1]
            if torch.any(double_support):
                double_support_foot_lateral_spacing.append(
                    foot_lateral_spacing[double_support].detach().clone()
                )
            for side in range(2):
    ...
    pelvis_roll = torch.stack(pelvis_roll_all)
    pelvis_pitch = torch.stack(pelvis_pitch_all)
    torso_roll = torch.stack(torso_roll_all)
    torso_pitch = torch.stack(torso_pitch_all)
    foot_lateral_spacing = torch.stack(foot_lateral_spacing_all)
    ...
    # Measure actual temporal participation per environment. Flattened standard
    # deviation would confuse randomized phase offsets with real joint motion.
    joint_temporal_std = sampled_joint.std(dim=0, correction=0).mean(dim=0)
    reference_joint_std = reference_joint_pos.std(dim=0, correction=0)
    joint_temporal_std_ratio = joint_temporal_std / torch.clamp(
        reference_joint_std, min=1.0e-4
    )
    phase_bins = []
    ...
    result = {
        ...
        "pelvis_roll_rad": {
            "signed": summary(pelvis_roll),
            "abs": summary(torch.abs(pelvis_roll)),
        },
        "pelvis_pitch_rad": {
            "signed": summary(pelvis_pitch),
            "abs": summary(torch.abs(pelvis_pitch)),
        },
        "torso_roll_rad": {
            "signed": summary(torso_roll),
            "abs": summary(torch.abs(torso_roll)),
        },
        "torso_pitch_rad": {
            "signed": summary(torso_pitch),
            "abs": summary(torch.abs(torso_pitch)),
        },
        "foot_lateral_spacing_m": summary(foot_lateral_spacing),
        "double_support_foot_lateral_spacing_m": optional_summary(
            double_support_foot_lateral_spacing
        ),
        ...
        "aligned_j4340p_motor_ratio_by_joint": (
            {
                metric: per_name_summary(
                    aligned_j4340_envelope[:, :, index], aligned_j4340_names
                )
                for index, metric in enumerate(
                    ("rated_torque", "peak_torque", "rated_speed", "max_speed", "peak_torque_speed")
                )
        ...
        "aligned_j4340p_peak_torque_speed_exceedance_fraction_by_joint": (
            per_name_exceedance(
                aligned_j4340_envelope[:, :, 4], aligned_j4340_names
            )
            if aligned_j4340_envelope is not None
            else None
        ),
        ...
        "joint_temporal_std_by_name_rad": {
            name: float(value.item())
            for name, value in zip(reference_joint_names, joint_temporal_std)
        },
        "reference_joint_std_by_name_rad": {
            name: float(value.item())
            for name, value in zip(reference_joint_names, reference_joint_std)
        },
        "joint_temporal_std_ratio_to_reference_by_name": {
            name: float(value.item())
            for name, value in zip(reference_joint_names, joint_temporal_std_ratio)
        },
