...
from isaaclab_tasks.manager_based.locomotion.sprite0615_tracking.config.sprite0615.flat_env_cfg import (
    ...
    SPRITE0825_PM01_4340_SHOULDERS_FULL_PEAK_ACTION_SCALE,
    SPRITE0825_WBT_PM01_4340_SHOULDERS_FULL_PEAK_CFG,
)
...
def _resolve_sprite0825_asset_usd() -> str:
    override = os.environ.get("SPRITE0825_ASSET_USD")
    repo_root = os.environ.get("SPRITE_RL_ROOT")
    candidates = [Path(override)] if override else []
    if repo_root:
        candidates.append(Path(repo_root) / "assets/sprite0825_sanitized_v4/sprite0825.usd")
    legacy = Path("sprite/sprite_isaaclab/assets/sprite0825_sanitized_v4/sprite0825.usd")
    candidates.extend((Path("/home/tony") / legacy, Path("/root") / legacy))
...
SPRITE0825_ASSET_USD = _resolve_sprite0825_asset_usd()
...
def _build_pm01_full_motor_envelope_actuators(
    source_cfg: ArticulationCfg = SPRITE0615_WBT_PM01_FULL_PEAK_CFG,
):
    """Keep PM01 delay while enforcing each physical motor's torque-speed line."""
    actuators = {}
    for name, cfg in source_cfg.actuators.items():
...
@configclass
class Sprite0825Stage2PM014340ShouldersFullMotorEnvelopeSceneCfg(
    Sprite0615Stage2PM01FullMotorEnvelopeSceneCfg
):
    """Sprite0825 v5 with J4340P shoulder pitch/roll and PM01 motor delay."""

    robot: ArticulationCfg = SPRITE0825_WBT_PM01_4340_SHOULDERS_FULL_PEAK_CFG.replace(
        prim_path="{ENV_REGEX_NS}/Robot",
        actuators=_build_pm01_full_motor_envelope_actuators(
            SPRITE0825_WBT_PM01_4340_SHOULDERS_FULL_PEAK_CFG
        ),
...
class Sprite0825Stage2AMPG57NativePM01Forward100HzRobustEnvCfg(
    ...
    def __post_init__(self):
        super().__post_init__()
        self.scene.robot.spawn.usd_path = SPRITE0825_ASSET_USD
...
@configclass
class Sprite0825Stage2AMPG59Native50HzRobustEnvCfg(
    Sprite0825Stage2AMPG58Preserve925GentleYaw100HzRobustEnvCfg
):
    """G59: native 50 Hz actor contract for the embedded deployment runtime."""
...
@configclass
class Sprite0825Stage2AMPG59Native50HzEnvCfg(
    Sprite0825Stage2AMPG59Native50HzRobustEnvCfg
):
    """Deterministic G59 evaluation task without training perturbations."""

    events: EventsCfg = EventsCfg()


@configclass
class Sprite0825Stage2G60TimeNormalizedWaistRewardsCfg(
    Sprite0615Stage2PM01ExactRewardsCfg
):
    """Use PM01 regularization in physical time and softly bound waist roll."""

    action_rate = RewTerm(
        func=mdp.action_rate_with_curriculum,
        weight=-0.06 * _ACTION_RATE_COMPENSATION,
        params={"start_scale": 0.1, "power": 0.8, "interval_epochs": 2400},
    )
    action_smoothness = RewTerm(
        func=mdp.action_smoothness_with_curriculum,
        weight=-0.04 * _ACTION_SMOOTHNESS_COMPENSATION,
        params={"start_scale": 0.1, "power": 0.8, "interval_epochs": 2400},
    )
    waist_roll_soft = RewTerm(
        func=mdp.joint_deviation_exp,
        weight=0.2,
        params={
...
@configclass
class Sprite0825Stage2AMPG60TimeNormalizedWaist50HzRobustEnvCfg(
    Sprite0825Stage2AMPG59Native50HzRobustEnvCfg
):
    """G60: correct G59 frame-rate regularization and reduce excess waist roll."""

    rewards: Sprite0825Stage2G60TimeNormalizedWaistRewardsCfg = (
        Sprite0825Stage2G60TimeNormalizedWaistRewardsCfg()
    )


@configclass
class Sprite0825Stage2AMPG60TimeNormalizedWaist50HzEnvCfg(
    Sprite0825Stage2AMPG60TimeNormalizedWaist50HzRobustEnvCfg
):
    """Deterministic G60 evaluation task without training perturbations."""

    events: EventsCfg = EventsCfg()


@configclass
class Sprite0825Stage2AMPG61StartExposure50HzRobustEnvCfg(
    Sprite0825Stage2AMPG60TimeNormalizedWaist50HzRobustEnvCfg
):
    """G61: increase PM01 stand-to-walk exposure without changing its reward MDP."""

    def __post_init__(self):
        super().__post_init__()
        command = self.commands.base_velocity
        command.resampling_time_range = (4.0, 4.0)
        command.rel_standing_envs = 0.3


@configclass
class Sprite0825Stage2AMPG61StartExposure50HzEnvCfg(
    Sprite0825Stage2AMPG61StartExposure50HzRobustEnvCfg
):
    """Deterministic G61 evaluation task without training perturbations."""

    events: EventsCfg = EventsCfg()


@configclass
class Sprite0825Stage2AMPG62TransitionExposure50HzRobustEnvCfg(
    Sprite0825Stage2AMPG61StartExposure50HzRobustEnvCfg
):
    """G62: increase start/stop exposure after G61 narrowly missed raw100."""

    def __post_init__(self):
        super().__post_init__()
        command = self.commands.base_velocity
        command.resampling_time_range = (3.0, 3.0)
        command.rel_standing_envs = 0.4


@configclass
class Sprite0825Stage2AMPG62TransitionExposure50HzEnvCfg(
    Sprite0825Stage2AMPG62TransitionExposure50HzRobustEnvCfg
):
    """Deterministic G62 evaluation task without training perturbations."""

    events: EventsCfg = EventsCfg()


@configclass
class Sprite0825Stage2AMPG72G71Expert4340Shoulders50HzRobustEnvCfg(
    Sprite0825Stage2AMPG60TimeNormalizedWaist50HzRobustEnvCfg
):
    """G72: PM01 50 Hz command MDP with G71 gait and v5 J4340P shoulders."""

    scene: Sprite0825Stage2PM014340ShouldersFullMotorEnvelopeSceneCfg = (
        Sprite0825Stage2PM014340ShouldersFullMotorEnvelopeSceneCfg(
            num_envs=4096, env_spacing=2.5
        )
    )
    ...
    def __post_init__(self):
        super().__post_init__()
        # G57's compatibility hook selects the frozen v4 asset. G72 must
        # reassert the isolated v5 geometry after the full parent chain runs.
        self.scene.robot.spawn.usd_path = (
            SPRITE0825_WBT_PM01_4340_SHOULDERS_FULL_PEAK_CFG.spawn.usd_path
        )


@configclass
class Sprite0825Stage2AMPG72G71Expert4340Shoulders50HzEnvCfg(
    Sprite0825Stage2AMPG72G71Expert4340Shoulders50HzRobustEnvCfg
):
    """Deterministic G72 evaluation task without training perturbations."""

    events: EventsCfg = EventsCfg()


@configclass
class Sprite0825Stage2AMPG73ShoulderScaleFlatFeet50HzRobustEnvCfg(
    Sprite0825Stage2AMPG72G71Expert4340Shoulders50HzRobustEnvCfg
):
    """G73: apply the G71 proximal-shoulder action scale and strengthen flat feet."""

    def __post_init__(self):
        super().__post_init__()
        scale = copy.deepcopy(SPRITE0615_PM01_EXPLICIT_ACTION_SCALE)
        del scale[".*_shoulder_.*"]
        scale.update(
            {
        ...
        self.actions.joint_pos.scale = scale
        self.rewards.feet_orientation.weight = 0.5


@configclass
class Sprite0825Stage2AMPG73ShoulderScaleFlatFeet50HzEnvCfg(
    Sprite0825Stage2AMPG73ShoulderScaleFlatFeet50HzRobustEnvCfg
):
    """Deterministic G73 evaluation task without training perturbations."""

    events: EventsCfg = EventsCfg()


@configclass
class Sprite0825Stage2AMPG74ShoulderRollCenter50HzRobustEnvCfg(
    Sprite0825Stage2AMPG73ShoulderScaleFlatFeet50HzRobustEnvCfg
):
    """G74: align shoulder-roll reset/action centers with the qualified G71 reference."""

    def __post_init__(self):
        super().__post_init__()
        self.scene.robot.init_state.joint_pos.update(
            {
                "left_shoulder_roll_joint": 0.082816130,
                "right_shoulder_roll_joint": -0.076321766,
            }
        )


@configclass
class Sprite0825Stage2AMPG74ShoulderRollCenter50HzEnvCfg(
    Sprite0825Stage2AMPG74ShoulderRollCenter50HzRobustEnvCfg
):
    """Deterministic G74 evaluation task without training perturbations."""
