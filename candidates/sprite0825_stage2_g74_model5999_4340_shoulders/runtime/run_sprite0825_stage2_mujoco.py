...
def main() -> None:
    ...
    j4340_cfg = contract["physical_j4340p"]
    j4340_names = list(j4340_cfg["joint_names"])
    j4340_indices = np.asarray([joint_names.index(name) for name in j4340_names])
    j4340_rated_torque = float(j4340_cfg["rated_torque_nm"]) * args.motor_torque_scale
    j4340_rated_speed = float(j4340_cfg["rated_speed_rad_s"])
    j4340_peak_torque = np.full(
        len(j4340_names),
        float(j4340_cfg["peak_torque_nm"]) * args.motor_torque_scale,
    )
    j4340_no_load_speed = np.full(
        len(j4340_names), float(j4340_cfg["no_load_speed_rad_s"])
    )
    if not np.allclose(effort[j4340_indices], j4340_peak_torque, atol=1.0e-6):
        raise RuntimeError("J4340P peak torque differs from joint effort limits")
    if not np.allclose(velocity_limit[j4340_indices], j4340_no_load_speed, atol=1.0e-3):
        raise RuntimeError("J4340P no-load speed differs from joint velocity limits")
    ...
    sole_geom_ids = {
        side: mujoco.mj_name2id(model, mujoco.mjtObj.mjOBJ_GEOM, f"{side}_sole")
    ...
    if min(*sole_geom_ids.values(), *sole_body_ids.values()) < 0:
        raise RuntimeError("MJCF is missing named sole geoms or ankle roll bodies")
    data.qpos[qpos_addr] = default_q
    mujoco.mj_forward(model, data)

    def sole_contact_loads() -> dict[str, dict[str, object]]:
        """Return force-weighted sole contact locations in each foot frame."""
        result = {}
        for side in ("left", "right"):
            geom_id = sole_geom_ids[side]
            body_id = sole_body_ids[side]
            ...
            for contact_index in range(data.ncon):
                ...
                if geom_id not in (int(contact.geom1), int(contact.geom2)):
                    continue
                force = np.zeros(6, dtype=np.float64)
                mujoco.mj_contactForce(model, data, contact_index, force)
                normal_force = abs(float(force[0]))
                normal_forces.append(normal_force)
                positions_local.append(rotation.T @ (contact.pos - position))
            total_normal = float(sum(normal_forces))
            if total_normal > 1.0e-9:
                points = np.asarray(positions_local)
                weights = np.asarray(normal_forces)
                center = np.average(points, axis=0, weights=weights)
                lateral_abs = float(np.average(np.abs(points[:, 1]), weights=weights))
            else:
                center = np.full(3, np.nan)
                lateral_abs = math.nan
            result[side] = {
                "contact_count": len(normal_forces),
                "normal_force_n": total_normal,
                "cop_local_m": center.tolist(),
                "force_weighted_abs_lateral_m": lateral_abs,
            }
        return result
                ...
                trace.append(
                    {
                        ...
                        "sole_contact_load": sole_contact_loads(),
                    }
                )
